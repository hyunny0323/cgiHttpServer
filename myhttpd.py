...
def getFile(fileName):
    ...
    return (code, body)
def isempty_qString(qString):
    qVal = parse_qs(qString)
    if not qVal:    # 두 변수 empty
        qVal['name'] = "Empty"
        qVal['passwd'] = "Empty"
    ...
    if 'name' not in qVal:
        qVal['name'] = "Empty"
    if 'passwd' not in qVal:
        qVal['passwd'] = "Empty"

    qString = urlencode(qVal, doseq=True)
...
def doCGI(cgiProg, qString):
    ...
    qString = isempty_qString(qString)
...
def doCGI_post(cgiProg, qString):
    prog = './' + cgiProg
    print("prog(POST) : ", prog)
    ...
    try:
        ...
        code = '500'
        body = '<HTML><HEAD><link rel="short icon" href="#"></HEAD>' \
                       '<BODY><H1>500 POST Internal Sever Error</H1></BODY></HTML>'
        pass
    return (code, body)
...
def doHTTPService(sock) :
    ...
    if method == 'GET':
        ...
        if "\r\n\r\n" in msgString:
            temp = msgString.find("\r\n\r\n")
            qString = msgString[temp + 4:]
            print(qString)

        fileName = fields[1]
        fileName = fileName[1:] # /login_post.cgi => login_post.cgi
        code, responseBody = doCGI_post(fileName, qString)
    else:
        code = '501'
